...
class TestPortfolioCalculations:
    ...
    def test_mixed_active_and_closed_positions(
        ...
        total = portfolio.calculate_total_value()
        # Only active position (4000) + cash (2000) = 6000
...
class TestPortfolioSerialization:
    ...
    def test_to_dict_none_dates(self, sample_portfolio):
        portfolio = sample_portfolio(create_date=None)
        portfolio.last_maint = None
        d = portfolio.to_dict()
        assert d["create_date"] is None
        assert d["last_maint"] is None

    def test_to_dict_with_dates(self, sample_portfolio):
        today = date.today()
        portfolio = sample_portfolio(create_date=today)
        portfolio.last_maint = today
        d = portfolio.to_dict()
...
class TestHistoryAuditRecord:
    ...
    def test_create_audit_record_serializes_json(self):
    ...
    def test_create_audit_record_increments_seq_no(self, db_session, sample_portfolio):
        ...
        # Freeze datetime.now() so both records get the same date/time strings
        from unittest.mock import patch

        frozen_now = datetime(2026, 1, 15, 10, 30, 0, 123456)
        with patch("models.history.datetime") as mock_dt:
            ...
            # Create second record — same portfolio_id, date, time
        ...
        # The seq_no should be incremented
        assert record1.seq_no == "0001"
    ...
    def test_get_after_data_valid_json(self):
        record = History(after_image='{"status": "C"}')
